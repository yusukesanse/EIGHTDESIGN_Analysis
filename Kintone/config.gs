...
const RANK_MAP = {
  '不明':   'B-D',
  '面識なし': 'B-D',   // 見込み度が未判定 → 「不明」と同じ B-D 扱い
  '長期追客': 'C',
  '見込み無': 'D',
  '見込み小': 'B-C',
  '見込み中': 'B-B',
...
/**
 * 年度の切れ目。この月日以降の反響は「翌年度」として扱う。
 *
 * エイトデザインの年度は9月20日締め。9/20までが今年度で、9/21以降は翌年度。
 * 例（既定値の場合）: 2026-09-20 → 2026年度 / 2026-09-21 → 2027年度 /
 * 2026-12-05 → 2027年度 / 2027-03-01 → 2027年度。
 *
 * 一覧A列（反響年）の付与と、集計の「今年度」判定の両方がこの1か所を見る。
 * 年度の切れ目を変えるときはここだけを変更する。
 *
 * @type {{ month: number, day: number }}
 */
const YEAR_ROLLOVER = { month: 9, day: 21 };
